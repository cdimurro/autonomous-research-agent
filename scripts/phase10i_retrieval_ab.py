#!/usr/bin/env python
"""Phase 10I: Confirmatory limited retrieval A/B after ranking & persistence fixes.
...
generation) with Phase 10I fixes applied:

  - Diversity-aware evidence ranking (select_diverse_top_k)
  - Evidence item persistence repair (fresh IDs per pack)
  - Expanded diversity panel with pre/post-ranking metrics
...
logger = logging.getLogger("phase10i")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(name)s %(message)s")

OUTDIR = os.path.join(ROOT, "runtime", "phase10i")
...
# ---------------------------------------------------------------------------
# Campaign result dataclass (expanded for Phase 10I)
# ---------------------------------------------------------------------------
...
class CampaignResult:
    ...
    # Phase 10I: persistence and ranking audit fields
    ...
    for i in range(CAMPAIGNS_PER_ARM):
        ...
        try:
            ...
            if result.run_id:
                ...
                # Phase 10I: Evidence pack persistence audit
                try:
    ...
    for camp in campaigns:
        ...
        for rank, (cand, score) in enumerate(scored_candidates[:2]):
            ...
            label = {
                ...
                "reviewer_note": f"phase10i_auto_{camp.arm}",
                "reviewer": "phase10i_automated",
                "arm": camp.arm,
            }
...
# ---------------------------------------------------------------------------
# Comparison (expanded for Phase 10I persistence and ranking audit)
# ---------------------------------------------------------------------------
...
def export_artifacts(
    ...
    lines = [
        "# Phase 10I: Confirmatory Retrieval A/B — Comparison Summary",
        "",
        ...
        f"**Ranking:** diversity_aware_v1 (Phase 10I fix)",
        f"**Persistence:** fresh_ids_per_pack (Phase 10I fix)",
        f"**Graph caching:** enabled",
        "",
        "## Fixes Applied (Phase 10I)",
        "",
        "1. **Diversity-aware ranking**: `select_diverse_top_k()` prevents",
        "   ranking-layer concentration by enforcing per-source cap in top-k selection.",
        "2. **Evidence item persistence**: Fresh item IDs per pack prevent",
        "   `INSERT OR REPLACE` collisions when multiple packs share evidence.",
        "",
        "## Arm Comparison",
        "",
    ...
    with open(f"{OUTDIR}/label_completion_summary.md", "w") as f:
        f.write("# Phase 10I: Label Completion Summary\n\n")
...
def write_comparability_report():
    ...
    report = {
        ...
        "changes_from_phase10h": {
            "ranking": "diversity_aware_v1 (select_diverse_top_k with max_per_source=1)",
            "persistence": "fresh item IDs per pack (prevents INSERT OR REPLACE collision)",
            "campaigns_per_arm": "7 (same as 10H)",
        },
...
def main():
    ...
    print("=" * 60)
    print("Phase 10I: Confirmatory Retrieval A/B (7+7)")
    print("DIVERSITY-AWARE RANKING + PERSISTENCE FIX")
    ...
    print(f"Changes from 10H: {json.dumps(comp_report['changes_from_phase10h'], indent=2)}")
