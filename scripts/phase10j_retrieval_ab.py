#!/usr/bin/env python
"""Phase 10J: Confirmatory limited retrieval A/B after pool diversification fixes.
...
generation) with Phase 10J fixes applied:

  - Phase 10J: Diversity-aware evidence ranking (select_diverse_top_k)
  - Phase 10J: Evidence item persistence repair (fresh IDs per pack)
  - Phase 10J: evidence_refs diversity check (fall through to ranking if non-diverse)
  - Phase 10J: Source-aware hybrid pool construction (KG item reservation, per-paper cap)
...
logger = logging.getLogger("phase10j")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(name)s %(message)s")

OUTDIR = os.path.join(ROOT, "runtime", "phase10j")
...
# ---------------------------------------------------------------------------
# Campaign result dataclass (expanded for Phase 10J)
# ---------------------------------------------------------------------------
...
class CampaignResult:
    ...
    # Phase 10J: persistence and ranking audit fields
    ...
    for i in range(CAMPAIGNS_PER_ARM):
        ...
        try:
            ...
            if result.run_id:
                ...
                # Phase 10J: Evidence pack persistence audit
                try:
            ...
            print(f" done ({elapsed:.0f}s) packs={result.packs_with_items}/{result.total_packs} — {result.champion_title[:55]}")
    ...
    for camp in campaigns:
        ...
        for rank, (cand, score) in enumerate(scored_candidates[:2]):
            ...
            label = {
                ...
                "reviewer_note": f"phase10j_auto_{camp.arm}",
                "reviewer": "phase10j_automated",
                "arm": camp.arm,
            }
...
# ---------------------------------------------------------------------------
# Comparison (expanded for Phase 10J persistence and ranking audit)
# ---------------------------------------------------------------------------
...
def compare_arms(
    ...
    """Compare the two retrieval arms with expanded metrics."""
    ...
    def arm_stats(campaigns, arm_labels):
        ...
        return {
            ...
            # Phase 10J persistence metrics
            "mean_persistence_rate": round(
                sum(c.persistence_rate for c in completed) / len(completed), 3
            ) if completed else 0,
            "mean_packs_with_items": round(
                sum(c.packs_with_items for c in completed) / len(completed), 1
            ) if completed else 0,
            "mean_total_packs": round(
                sum(c.total_packs for c in completed) / len(completed), 1
            ) if completed else 0,
    ...
    if all_pass and promotion_checks:
        ...
        reason = "All thresholds pass after ranking fix. Graph-native retrieval meets quality bar."
    ...
    return {
        ...
        "ranking_version": "diversity_aware_v1",
        "persistence_fix": "fresh_ids_per_pack",
        "evidence_refs_fix": "diversity_fallthrough_v1",
        "pool_construction": "source_aware_v1",
        "graph_caching": True,
...
def export_artifacts(
    ...
    with open(f"{OUTDIR}/champions.csv", "w", newline="") as f:
        ...
        w.writerow(["arm", "campaign_num", "campaign_id", "champion_title",
                     ...
                     "top_concentration", "packs_with_items", "total_packs",
                     "persistence_rate", "evidence_source", "graph_conditioned"])
    ...
    with open(f"{OUTDIR}/campaign_metrics.csv", "w", newline="") as f:
        ...
        w.writerow(["arm", "campaign_num", "campaign_id", "status",
                     ...
                     "packs_with_items", "total_packs", "persistence_rate",
    ...
    lines = [
        "# Phase 10J: Confirmatory Retrieval A/B — Comparison Summary",
        "",
        ...
        f"**Diversity metric:** segment_level_v2",
        f"**Ranking:** diversity_aware_v1 (Phase 10J fix)",
        f"**Persistence:** fresh_ids_per_pack (Phase 10J fix)",
        f"**Graph caching:** enabled",
        "",
        "## Fixes Applied (Phase 10J)",
        "",
        "1. **Evidence_refs diversity check**: Non-diverse evidence_refs fall through",
        "   to ranked matching so select_diverse_top_k can enforce source diversity.",
        "2. **Source-aware hybrid pool**: KG items guaranteed in pool via slot reservation;",
        "   per-paper cap prevents any single paper from dominating the pool.",
        ...
        f"| Mean persistence rate | {ca['mean_persistence_rate']:.1%} | {ga['mean_persistence_rate']:.1%} | |",
        f"| Mean packs w/ items | {ca['mean_packs_with_items']} | {ga['mean_packs_with_items']} | |",
    ...
    lines += [
        ...
        "",
        "| # | Score | Finalists | Sources | Persistence | Champion |",
        "|---|-------|-----------|---------|-------------|---------|",
    ]
    for r in current_results:
        if r.status != "failed":
            lines.append(
                ...
                f"{r.unique_sources} | "
                f"{r.packs_with_items}/{r.total_packs} | "
    ...
    with open(f"{OUTDIR}/label_completion_summary.md", "w") as f:
        f.write("# Phase 10J: Label Completion Summary\n\n")
...
def write_comparability_report():
    ...
    report = {
        ...
        "changes_from_phase10j": {
            "evidence_refs": "diversity_fallthrough_v1 (fall through to ranking when refs are non-diverse)",
            "pool_construction": "source_aware_v1 (KG item reservation, per-paper cap)",
            "campaigns_per_arm": "7 (same as 10I)",
        },
...
def main():
    ...
    print("=" * 60)
    print("Phase 10J: Confirmatory Retrieval A/B (7+7)")
    print("EVIDENCE_REFS FIX + SOURCE-AWARE POOL CONSTRUCTION")
    ...
    print(f"Changes from 10I: {json.dumps(comp_report['changes_from_phase10j'], indent=2)}")
    ...
    print(f"  Persistence rate:  {ca['mean_persistence_rate']:.1%}")
    ...
    print(f"  Persistence rate:  {ga['mean_persistence_rate']:.1%}")
